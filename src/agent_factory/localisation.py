...
CATALOGUE: Mapping[str, Message] = {
    ...
    "studio.create.notready": Message("Підключіть локальну модель Ollama на сторінці «Доступ до AI», потім поверніться до цієї гри й продовжіть.",
                                      "Connect the local Ollama model on AI access, then return to this game and continue."),
    ...
    "studio.updates.title": Message("Оновлення студії", "Studio updates"),
    "studio.updates.check": Message("Перевірити оновлення", "Check for updates"),
    "studio.updates.download": Message("Завантажити EXE для Windows", "Download Windows EXE"),
    "studio.updates.idle": Message("Перевірка відбувається лише після натискання. AI для цього не використовується.", "Updates are checked only when you click. No AI is used."),
    "studio.updates.installed": Message("Ваша версія: {version}", "Your version: {version}"),
    "studio.updates.source": Message("Студію запущено з вихідного коду. Тут можна завантажити опублікований інсталятор для іншого ПК.", "Studio is running from source. You can download a published installer for another PC here."),
    "studio.updates.unknown": Message("Номер установленої версії невідомий. Перевірте доступний інсталятор.", "Installed version is unknown. Check the available installer."),
    "studio.updates.checking": Message("Перевіряємо офіційні релізи…", "Checking official releases…"),
    "studio.updates.update": Message("Є оновлення: {version}. Можна завантажити й установити.", "Update available: {version}. You can download and install it."),
    "studio.updates.current": Message("У вас актуальна опублікована версія: {version}.", "You have the current published version: {version}."),
    "studio.updates.ahead": Message("Ваша збірка новіша за опубліковану {version}. Оновлення не потрібне.", "Your build is newer than published version {version}. No update is needed."),
    "studio.updates.available": Message("Доступний інсталятор: {version}.", "Installer available: {version}."),
    "studio.updates.unavailable": Message("Не вдалося перевірити оновлення. Перевірте інтернет і повторіть або відкрийте офіційні завантаження нижче.", "Could not check for updates. Check your connection and retry, or open official downloads below."),
    "studio.updates.details": Message("Windows 64-bit · {size} МБ · Перевірено: {time}", "Windows 64-bit · {size} MB · Checked: {time}"),
    "studio.updates.how": Message("Як установити оновлення", "How to install the update"),
    "studio.updates.instructions": Message("Завершіть активну роботу й закрийте студію. Відкрийте завантажений EXE та натисніть «Встановити й відкрити». Видаляти попередню інсталяцію, ігри чи моделі не потрібно. Завантаження саме не встановлює оновлення.", "Finish active work and close Studio. Open the downloaded EXE and choose Install and open. You do not need to remove the previous installation, games or models. Downloading does not install the update."),
    "studio.updates.releases": Message("Відкрити офіційні завантаження", "Open official downloads"),
